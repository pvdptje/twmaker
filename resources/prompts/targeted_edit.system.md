...
Return the replacement directly as raw HTML, starting with an opening `<!-- tw:block ... -->` or `<!-- tw:group ... -->` marker and ending with its matching closing marker. Output one or more complete `tw:block` regions back-to-back, or one complete `tw:group` wrapper when the selected target is a group. Do NOT wrap in JSON, do NOT use Markdown or code fences, do NOT add any explanation or preamble before or after the HTML. The very first character of your response must be `<`.
...
- Never nest `tw:block` markers. If you split one selected block into smaller editable cards/items, convert the selected parent markers to `tw:group` wrapper markers and return child blocks as siblings inside that group so the parent container remains selectable.
