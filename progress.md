...
- Files touched: app/Events/GenerationStreamChunk.php, app/Livewire/Builder/Canvas/Canvas.php, app/Livewire/Builder/Workspace/Workspace.php, app/Livewire/Builder/Workspace/workspace.blade.php, app/Livewire/Builder/LeftSidebar/LeftSidebar.php, app/Livewire/Builder/LeftSidebar/left-sidebar.blade.php, app/Livewire/Projects/ProjectDashboard/ProjectDashboard.php, app/Models/Project.php, app/Services/Generation/Pipeline.php, app/Services/Generation/Stages/Planner.php, app/Services/Generation/Stages/SectionGenerator.php, app/Services/Generation/Stages/HtmlMarker.php, app/Services/Generation/Stages/TargetedEdit.php, app/Services/Generation/GenerationEventRecorder.php, app/Services/Generation/GenerationStreamBuffer.php, app/Livewire/Builder/StreamPanel/stream-panel.blade.php, app/Livewire/Builder/Inspector/EditForm/EditForm.php, app/Livewire/Builder/Inspector/EditForm/edit-form.blade.php, app/Livewire/Builder/SidePanels/GenerationControls/GenerationControls.php, app/Livewire/Builder/SidePanels/GenerationControls/generation-controls.blade.php, app/Livewire/Setup/LlmSetup.php, app/Livewire/Setup/llm-setup.blade.php, app/Services/Llm/DeepSeekProvider.php, app/Services/Llm/LlmManager.php, app/Services/Llm/LlmRegistry.php, app/Services/Llm/ProviderModelCatalog.php, config/llm.php, routes/web.php, .env.example, resources/prompts/planner.system.md, resources/prompts/section_generator.system.md, plan.md, tests/Feature/Generation/PipelineTest.php, tests/Feature/BuilderShellTest.php, tests/Unit/Llm/DeepSeekProviderTest.php, tests/Unit/Llm/LlmRegistryTest.php, tests/Unit/Llm/ProviderModelCatalogTest.php, progress.md
- Current state: Flexible block-level targeted editing is wired through the inspector and pipeline, with livelier stream/status feedback. Provider/model selection is UI-driven, keys are stored locally per provider, a dedicated setup page stores primary/editing defaults in the browser, model catalogs can be fetched and cached from providers, and token totals are shown per model. Browser localStorage keys are the primary open-source/self-host flow; env keys remain optional fallbacks for demos or shared installs. Builder-side provider/model dropdowns now stay in Alpine state until submit, avoiding repeated Livewire updates during workspace polling. Anthropic and DeepSeek are implemented providers; DeepSeek structured calls avoid thinking-mode tool-choice conflicts and text generation streams without tools. Section generation now asks for plain HTML text instead of structured JSON, while targeted edit and marker fallback still use structured tool calls where fields are useful. DeepSeek SSE payloads, streamed chunks, and generation-event payloads are scrubbed for valid UTF-8 before JSON decoding, cache, DB, or broadcast boundaries. DeepSeek text deltas are coalesced before stream handling, tolerate malformed provider events, strip Markdown code fences, and stream cleaned HTML into the modal. Live stream snapshots accumulate in memory and flush to cache at most about once per second plus stage end; final pages are still the only durable document writes. Recoverable inline event handlers and `javascript:` URLs are sanitized before final validation, while script tags remain validation failures. Derived block summaries and document arrays are UTF-8 safe before JSON persistence. Streamed HTML appears for full generation and targeted edits, each new run clears the modal stream before fresh output arrives, and the modal can be reopened while a job is running. Verified with `php artisan test`, `npm.cmd run build`, and `vendor\bin\pint.bat --dirty --test`.
