...
- Files touched: app/Livewire/Builder/StreamPanel/StreamPanel.php, app/Livewire/Builder/StreamPanel/stream-panel.blade.php, app/Livewire/Builder/Workspace/Workspace.php, app/Livewire/Builder/Workspace/workspace.blade.php, app/Services/Generation/Pipeline.php, app/Services/Generation/Stages/TargetedEdit.php, public/preview-bridge.js, resources/js/builder-canvas.js, tests/Feature/BuilderShellTest.php, tests/Feature/Generation/PipelineTest.php, tests/Js/preview-bridge.test.js, progress.md
- Current state: Flexible block-level targeted editing is wired through the inspector and pipeline, with livelier stream/status feedback. Streamed HTML appears for full generation and targeted edits, each new run clears the modal stream before fresh output arrives, and the modal can be reopened while a job is running. The stream modal now also closes from polled terminal page status when a broadcast completion event is missed, and targeted edits publish final `html_source` into the stream buffer if a provider returns structured output without streaming deltas. Preview-side targeted edit patching and selection overlay polish are present in the current worktree. Verified with `php artisan test --filter=BuilderShellTest`, `php artisan test --filter=PipelineTest`, and `vendor\bin\pint.bat --dirty --test`.
