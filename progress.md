...
- Current state: Flexible block-level targeted editing is wired through the inspector and pipeline, with livelier stream/status feedback. Provider/model selection is UI-driven, keys are stored locally per provider, a dedicated setup page stores primary/editing defaults in the browser, model catalogs can be fetched and cached from providers, and token totals are shown per model. Browser localStorage keys are the primary open-source/self-host flow; env keys remain optional fallbacks for demos or shared installs. Builder-side provider/model dropdowns now stay in Alpine state until submit, avoiding repeated Livewire updates during workspace polling. Anthropic and DeepSeek are implemented providers; DeepSeek structured calls avoid thinking-mode tool-choice conflicts and text generation streams without tools. DeepSeek SSE payloads, streamed chunks, and generation-event payloads are scrubbed for valid UTF-8 before JSON decoding, cache, DB, or broadcast boundaries. DeepSeek text deltas are coalesced before stream handling to reduce local per-chunk overhead. Live stream snapshots accumulate in memory and flush to cache at most about once per second plus stage end; final pages are still the only durable document writes. Streamed HTML appears for full generation and targeted edits, each new run clears the modal stream before fresh output arrives, and the modal can be reopened while a job is running. Verified with `php artisan test`, `npm.cmd run build`, and `vendor\bin\pint.bat --dirty --test`.
