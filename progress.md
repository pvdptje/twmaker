...
- Current state: Flexible block-level targeted editing is wired through the inspector and pipeline, with livelier stream/status feedback. Provider/model selection is UI-driven, keys are stored locally per provider, a dedicated setup page stores primary/editing defaults in the browser, model catalogs can be fetched and cached from providers, and token totals are shown per model. Anthropic and DeepSeek are implemented providers; DeepSeek structured calls avoid thinking-mode tool-choice conflicts and text generation streams without tools. Streamed HTML appears for full generation and targeted edits, each new run clears the modal stream before fresh output arrives, and the modal can be reopened while a job is running. Verified with `php artisan test`, `npm.cmd run build`, and `vendor\bin\pint.bat --dirty --test`.
